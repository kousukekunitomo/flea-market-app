...
@php
    ...
    $payOld   = old('payment_method', 'convenience_store');
...
<div class="purchase-wrap">
  ...
  {{-- フォーム全体 --}}
  <form method="POST" action="{{ route('purchase.store', $item) }}" class="purchase-grid" id="purchase-form">
    @csrf
    ...
    <div class="purchase-left">
      ...
      <div class="purchase-block pay-block">
        ...
        <div class="purchase-field">
          <select name="payment_method" class="purchase-select" id="js-pay">
            <option value="convenience_store" {{ $payOld==='convenience_store' ? 'selected' : '' }}>コンビニ払い</option>
            <option value="credit_card"       {{ $payOld==='credit_card'       ? 'selected' : '' }}>クレジットカード</option>
            <option value="bank_transfer"     {{ $payOld==='bank_transfer'     ? 'selected' : '' }}>銀行振込</option>
          </select>
        </div>
        @error('payment_method') <div class="purchase-error">{{ $message }}</div> @enderror
      </div>
